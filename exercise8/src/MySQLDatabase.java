...
public class MySQLDatabase {
  ...
  private static final java.util.Map<String, Integer> loginAttempts = new java.util.HashMap<>();
  ...
  public User login(String userId, String password) throws DLException {
    try {
        ...
        if (authenticated) {
            ...
            int attempts = loginAttempts.get(userId) + 1;
            loginAttempts.put(userId, attempts);
            
            System.out.println("Authentication failed: " + userId);
            System.out.println("Attempt no. " + attempts + " of " + MAX_LOGIN_ATTEMPTS);
            
            if (attempts >= MAX_LOGIN_ATTEMPTS) {
                System.out.println("Max login attempts made terminating ");
                System.exit(1);
            }
